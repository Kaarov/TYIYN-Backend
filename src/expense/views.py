...
from .models import ExpenseModel, models
from .serializers import ExpenseSerializer


@extend_schema(
    summary="Get Expenses by card id",
    description=("This endpoint returns expenses by card id. "),
    parameters=[
...
class ExpenseCardIdAPIView(views.APIView):
    def get(self, request):
        user = request.user
        card_id = request.query_params.get("card_id")

        queryset = ExpenseModel.objects.filter(user=user, card_id=card_id)
